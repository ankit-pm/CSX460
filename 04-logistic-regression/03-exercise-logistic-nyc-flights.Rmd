...
logisticFunc<- function(x){y<-(1)/(1+exp(-x))
return(y)}
x<--10:10
plot(x,logisticFunc(x),main = "Logistic Function for -10 to 10")
m<-0:1

InverseLogisticFunc<-function(m){InverseLogistic<-exp(m)/(1+exp(m))
+ return(InverseLogistic)}

b<-seq(from=0, to=1, by=0.02)
plot(b,logisticFunction(b),main = "Plot of Inverse Logistic")
...
InputData<-read.csv("NYCFlightmodel.csv")
InputData$arr_delayBoolean<-ifelse(InputData$arr_delay>=15,1,0)
InputData$arr_delayBoolean<-as.factor(InputData$arr_delayBoolean)
